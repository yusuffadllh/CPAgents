 ...
 * @param {boolean} [params.allowDeploy] Inject Vercel/Netlify tokens. Off by
 *   default so a mid-build task physically cannot deploy.
 */
export async function runOpencode({ prompt, cwd, settings, signal, onOutput, allowDeploy = false, idleTimeoutMs = 1200000, maxTimeoutMs = 3300000 }) {
  ...
  const cliDeployAllowed = allowDeploy && settings?.deployMode !== 'git';
  const env = {
    ...process.env,
    HOME: isolatedHome,
    ...
    // Withheld unless this run is the deploy step, and always in git mode where
    // the platform builds from the push: with no token `vercel deploy` cannot
    // succeed even if the model ignores the instructions, so a duplicate
    // deployment becomes impossible rather than merely discouraged.
    VERCEL_TOKEN: cliDeployAllowed ? (settings?.vercelToken || '') : '',
    NETLIFY_AUTH_TOKEN: cliDeployAllowed ? (settings?.netlifyToken || '') : '',
    // git identity, otherwise `git commit` aborts asking who the author is.
    GIT_AUTHOR_NAME: settings?.githubUsername || 'AI Agent',
    GIT_AUTHOR_EMAIL: settings?.githubEmail || 'agent@localhost',
    GIT_COMMITTER_NAME: settings?.githubUsername || 'AI Agent',
    GIT_COMMITTER_EMAIL: settings?.githubEmail || 'agent@localhost',
  ...
  // Store the PAT in $HOME/.git-credentials rather than in the remote URL: the
  // isolated HOME sits outside the workspace, so the token can't reach
  // .git/config, a commit, or the exported ZIP.
  if (settings?.githubToken) {
    const user = settings.githubUsername || 'x-access-token';
    try {
      fs.writeFileSync(
        path.join(isolatedHome, '.git-credentials'),
        `https://${encodeURIComponent(user)}:${encodeURIComponent(settings.githubToken)}@github.com\n`,
        { mode: 0o600 },
      );
      fs.writeFileSync(
        ...
        [
          ...
          '\thelper = store',
          '[user]',
          `\tname = ${settings.githubUsername || 'AI Agent'}`,
          `\temail = ${settings.githubEmail || 'agent@localhost'}`,
          '[init]',
          '\tdefaultBranch = main',
          '[safe]',
          '\tdirectory = *',
  ...
  // whole tree at once; without it a kill only reaps `script` and leaves
  // opencode plus its npm/node children running.
  const spawnOpts = { cwd, env, shell: false, detached: process.platform !== 'win32' };
  ...
  return new Promise((resolve, reject) => {
    ...
    // SIGTERM lets opencode flush and exit cleanly; escalate only if it ignores us.
    const stopTree = () => {
      killTree('SIGTERM');
    ...
    const onAbort = () => {
      onOutput('⛔ Stop diminta — menghentikan OpenCode beserta proses turunannya...');
      stopTree();
    };
    ...
    const shellCmd = [bin, ...args]
      ...
      .join(' ');
    try {
      child = spawn('script', ['-qec', shellCmd, '/dev/null'], spawnOpts);
    } catch {
      child = spawn(bin, args, spawnOpts);
      wireChild(false);
      resetIdle();
